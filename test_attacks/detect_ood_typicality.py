...
# NOTE: logsumexp is no longer needed as we transition from P(x) to Typicality Test
...
# NOTE: model_eval is not used for OOD detection
...
def compute_empirical_entropy(logit):
    ...
    Since logit is typically the log-likelihood output of the model (log p(x)),
    this simply returns the negative log-likelihood scores.

    The mean of these scores is the entropy estimator (Eq. 5 or Eq. 4 in paper).
    """
    # NOTE: The provided model likelihood is log p(x). The paper's test uses -log p(x)
    # The input logit from the VAE/Flow model is the log-likelihood (log p(x))
    # For a generative model, the likelihood p(x) is typically calculated, and the
    # model output y_logit_op is the log-likelihood, log p(x).
    # The quantity used in the test is the negative log-likelihood.
    # We must compute -log p(x) = -logit.
    
    # log p(x) for each sample is the input logit
    # We return the negative log-likelihoods for use in the test statistic
    neg_logpx = -logit
    return neg_logpx
...
def compute_test_statistic(neg_logpx_batch, resubstitution_entropy_hat):
    ...
    neg_logpx_batch: Negative log-likelihoods for a batch of M samples (size M).
    resubstitution_entropy_hat: The pre-calculated model entropy estimate (scalar).
    """
...
def test_ood(batch_size, guard_name, data_name, save, alpha=0.99, K_bootstrap=50):
    ...
    # The paper's suggestion: hold out X' from the test set for bootstrap.
    ...
    # y placeholder is not strictly needed for likelihood-based OOD test
    ...
    for k in xrange(K_bootstrap):
        ...
        # np.random.choice will handle the bootstrap sampling (with replacement)
    ...
    # Threshold is the alpha-quantile of the bootstrap distribution
    epsilon_alpha_M = np.quantile(epsilon_hat_bootstrap, alpha)
    print('Rejection Threshold (epsilon_%.2f^M): %.3f' % (alpha, epsilon_alpha_M))
    print("-------------------------------------")

    # --- 5. OOD Detection (Online steps) ---
    
    # 5.1 Compute test statistics for clean ID and OOD samples
    # We must process the samples in M-sized batches to compute the statistic correctly

    def detect_typicality_rate(data_x, is_ood_set=False):
        """Computes the fraction of M-sized batches classified as OOD."""
        n_samples = data_x.shape[0]
        # Only use full batches that are perfectly M-sized for cleaner results
        n_batches = int(n_samples / M)
        batches_classified_ood = 0
        
        for i in xrange(n_batches):
            X_batch = data_x[i*M:(i+1)*M]
            
            # Compute -log p(x) for the batch
            logit_batch = sess.run(y_logit_op, feed_dict={x: X_batch})
            ...
            # OOD decision: reject if epsilon_hat is greater than the threshold
            if epsilon_hat > epsilon_alpha_M:
                batches_classified_ood += 1
        
        # Calculate the fraction of batches classified as OOD
        detection_rate = (batches_classified_ood / n_batches) * 100 if n_batches > 0 else 0.0
    ...
    fp_rate = detect_typicality_rate(x_clean, is_ood_set=False)
    
    # 5.3 Calculate TP Rate (OOD Detection) - Should be high
    tp_rate_ood = detect_typicality_rate(x_ood, is_ood_set=True)
    ...
    # --- 6. Save Results ---
    if save:
        # ... (save logic, same as original code)
...
if __name__ == '__main__':
    ...
    # The batch_size (-B) is the sequence length M in the paper's typicality test
    ...
    # Typicality Test specific arguments
    ...
    # NOTE: The data_name is still 'plane_frog' to ensure ID labels are [0, 6]
